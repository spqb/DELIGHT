...
for dataset in datasets_list:
    ...
    for seed in seeds:
        for t1 in t1_values:
            print(f"--> Using t1: {t1}, seed: {seed}")
            dirname = f"./experiments/datasets/{dataset}/t1{t1}_t20.7_repeated"
            
            # import test data
            test_df = pd.read_csv(os.path.join(dirname, f"seed_{seed}.test.csv"))
            ...
            for num_train_samples in num_train_samples_dict[dataset]:
                ...
                # import train data
                train_df = pd.read_csv(os.path.join(dirname, f"seed_{seed}.train_{num_train_samples}.csv"))
                ...
                # save results
                np.savez_compressed(
                    os.path.join(dirname, f"seed_{seed}.test.msa_{num_train_samples}.npz"),
                    labels=y_pred,
                    probs=y_pred_proba,
    ...
    for seed in seeds:
        for t1 in t1_values:
            ...
            for num_train_samples in num_train_samples_dict[dataset]:
                ...
                # save results
                np.savez_compressed(
                    os.path.join(dirname, f"seed_{seed}.test.rbm_{num_train_samples}.npz"),
                    labels=y_pred,
                    probs=y_pred_proba,
                    headers=test_df["header"].values,
                    embeddings=H_test,
